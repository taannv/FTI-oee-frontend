...
import {getDataCanOng01,getDataProductionOrderCanOng01,getStatusCanOng01,getSpeedCanOng01} from '../../services/CanOng01Service';
...
class CanOng01Home extends Component{
    constructor(props) {
        super(props);
        const data = {};
    ...
    componentDidMount() {
        getDataCanOng01().then(data=>this.setState({split1:data}));
        getDataProductionOrderCanOng01().then(data=>{
            this.setState({production1:data})
        });
        getStatusCanOng01().then(data=>{
            this.setState({status_on:data.status});
        });
        getSpeedCanOng01().then(data=>{
            this.setState({speed:data.speed})
        });
       
        this.timer = setInterval(()=>{
            getDataCanOng01().then(data=>this.setState({split1:data}));
            getDataProductionOrderCanOng01().then(data=>this.setState({production1:data}));
            getStatusCanOng01().then(data=>this.setState({status_on:data.status}));
            getSpeedCanOng01().then(data=>this.setState({speed:data.speed}));
        },2000)
                                ...
                                <Card title="Lệnh sản xuất" >
                                    <Descriptions size={'small'} bordered>
                                        <Descriptions.Item label="Mã Lệnh SX" span={3}>
                                            {this.state.production1.id}
                                        </Descriptions.Item>
                                        <Descriptions.Item label="S.Lượng cần SX (cây)" span={3}>
                                            <FormattedNumber value={this.state.split1.slkh} />
                                        </Descriptions.Item>
                                    </Descriptions>
                                </Card>
                            </Col>
                                ...
                                <Card title="Nguyên Liệu Vào">
                                    <Descriptions size={'small'} bordered>
                                        <Descriptions.Item label="Độ dày tôn (mm)" span={3}>
                                            <FormattedNumber value= {this.state.production1.day_in} />
                                        </Descriptions.Item>
                                        <Descriptions.Item label="Khổ băng (mm)" span={3}>                                            
                                            <FormattedNumber value={this.state.production1.rong_in}/>
                                        </Descriptions.Item>
                                    </Descriptions>
                                </Card>
                            </Col>
                                ...
                                <Card title="Quy cách đầu ra">
                                    <Descriptions size={'small'} bordered>
                                        <Descriptions.Item label="Chiều rộng (mm)" span={3}>
                                            <FormattedNumber value= {this.state.production1.rong_out} />
                                        </Descriptions.Item>
                                        <Descriptions.Item label="Chiều cao (mm)" span={3}>                                            
                                            <FormattedNumber value={this.state.production1.cao_out}/>
                                        </Descriptions.Item>
                                        <Descriptions.Item label="Chiều dài (mm)" span={3}>                                            
                                            <FormattedNumber value={this.state.production1.dai_out}/>
                                        </Descriptions.Item>
                                    </Descriptions>
                                </Card>
                            </Col>
                    ...
                    <Col span={9}>
                        ...
                        <Descriptions size={'small'} bordered>
                                    <Descriptions.Item label="Số lượng đã SX(cây)" span={3}>
                                            <FormattedNumber value= {this.state.split1.slsx} />
                                        </Descriptions.Item>
                                        <Descriptions.Item label="Khối lượng đã SX(kg)" span={3}>
                                            <FormattedNumber value= {this.state.split1.kltt8t + this.state.split1.kltttc} />
                                        </Descriptions.Item>
                                        <Descriptions.Item label="S.Lượng chính phẩm (cây)" span={3}>                                            
                                            <FormattedNumber value={this.state.split1.chinhpham}/>
                                        </Descriptions.Item>
                                        <Descriptions.Item label="S.Lượng thứ phẩm (kg)" span={3}>                                            
                                            <FormattedNumber value={this.state.split1.thupham}/>
                                        </Descriptions.Item>
                                        <Descriptions.Item label="S.Lượng phế phẩm (kg)" span={3}>                                            
                                            <FormattedNumber value={this.state.split1.phepham}/>
                                        </Descriptions.Item>
                                    </Descriptions>
                        </Card>
                    </Col>
