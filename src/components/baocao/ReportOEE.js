...
import {getDataMachineSplit} from '../../services/MachineService';
...
class ReportOEE extends React.Component{
    ...
    getDataOEE = ()=>{
        getDataMachineSplit().then(data=>{
            console.log(data);
            let dataColumn = [data.plan,data.produce];
            this.setState({
                data,
        ...
        return(
            <div>
                <Row gutter={8} >
                    ...
                    <Col span={12}>
                        <Card bordered={false}>
                        <MachineVolume data={this.state.dataColumn} categories={['Khối lượng theo kế hoạch', 'Khối lượng đã sản xuất']} height={250} width={300} />
                        </Card>
                    </Col>                                     
