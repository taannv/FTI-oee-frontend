...
import {reportErrorCO01,exportExcel} from '../../services/CanOng01Service';
...
class LoiDungMayCO01 extends React.Component{
    constructor(props) {
        super(props);                
        this.state = {
            data,
    ...
    loadPage=(offset,from,to)=>{
        ...
        reportErrorCO01(params).then(page=>{
            this.setState({
                loadingbtn1:false,
                data:page.content,
    ...
    onExportExcel= async()=>{
        ...
        for(let i = 0; i<this.state.totalPages;i++){            
            ...
            await reportErrorCO01(params).then(data=>{        
                console.log(data);    
            ...
            if(i==this.state.totalPages-1){
                exportExcel(dataXlsx,'log_error_C1');
        ...
        return(
            <Card title="LỖI DỪNG MÁY" bordered={false}>
                ...
                <Row gutter={8} style={{marginBottom:20,marginTop:20}}>
                    ...
                    <Col span={3}>
                        <br/>
                        <Button type="danger" loading={this.state.loadingbtn1} size={'small'} onClick={this.getReportError}><Icon type="read" />Xem Báo Cáo</Button>
                    </Col>
