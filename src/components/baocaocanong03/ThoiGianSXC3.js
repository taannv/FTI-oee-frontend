...
import {reportC3,exportExcel} from '../../services/CanOng03Service';
...
class ThoiGianSXC3 extends React.Component{
    constructor(props) {
        super(props);                
        this.state = {
            data,
     ...
     onChangeTo=(date, dateString)=> {
       ...
       exportXslx= async ()=>{
        ...
        for(let i = 0 ; i < this.state.totalPages; i++){          
          ...
          await reportC3(params).then(data=>{
              console.log(data);
          ...
          if(i==this.state.totalPages-1){
            ...
            exportExcel(dataXlsx,'thoi_gian_sx_C3');
          }   
        }
      }
        ...
        return(
            <Card title="BÁO CÁO THỜI GIAN SẢN XUẤT" bordered={false}>
                ...
                <Row gutter={8} style={{marginBottom:20,marginTop:20}}>
                    ...
                    <Col span={4}>
                        <label> </label><br/>
                        <Button type="danger" loading={this.state.loadingViewReport} size={'small'} onClick={this.getReportProductC3}><Icon type="read" />Xem Báo Cáo</Button>
                    </Col>
                    <Col span={4}>
                        <label> </label><br/>
